...
for i in range(3):
    print("Enter the 3d point for point P" + str(i + 1))
    for j in range(3):
        if j == 0:
...
for i in range(3):
    print("P" + str(i + 1) + ": " + str(points[i]))

print("\n")
for i in range(3):
    print("P" + str(i + 1) + "'s nearest point is P" + str(nearest_neighbors_list[i][1]) + " with the distance of {:.2f}".format(nearest_neighbors_list[i][0]))